...
export async function showBibmanPopup(plugin: BibmanPlugin, cite: HTMLElement): Promise<void> {
  const key = cite.dataset.bibkey;
  if (!key) return;

  const entry = await plugin.getBibEntry(key);
  hideBibmanPopup();
...
export function hideBibmanPopup(): void {
  activePopup?.remove();
  activePopup = null;
}

export function openBibmanNote(plugin: BibmanPlugin, cite: HTMLElement): void {
  const key = cite.dataset.bibkey;
  if (!key) return;
    ...
    this.containerEl
      ...
      .forEach((cite) => {
        this.registerDomEvent(cite, "mouseenter", () =>
          void showBibmanPopup(this.plugin, cite),
        );
        this.registerDomEvent(cite, "mouseleave", () => hideBibmanPopup());
        this.registerDomEvent(cite, "click", () => openBibmanNote(this.plugin, cite));
      });
  }
